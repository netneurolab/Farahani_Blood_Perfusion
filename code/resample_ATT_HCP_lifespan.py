...
# Resample arterial arrival time maps from 32k fsLR to 4k fsLR resolution.

## Output Files:
    down-sampled cortical data:
    4k_arrival_all_subjects_dev.npy
    4k_arrival_all_subjects_aging.npy

    down-sampled sub-cortical data:
    4k_subcortex_arrival_all_subjects_dev.npy
    4k_subcortex_arrival_all_subjects_aging.npy
...
arrival_file_name = 'pvcorr_arrival_Atlas_MSMAll'
...
for s, subid in enumerate(subject_id_dev):
    ...
    arrival_file = os.path.join(path_data_blood, arrival_file_name + '.dscalar.nii')
    img_arrival = nib.cifti2.load(arrival_file)
    arrival_cortical = img_arrival.get_data()[:, :globals.num_cort_vertices_noMW]

    arrival_cortical_32k = np.empty((globals.num_cort_vertices_withMW, 1))
    arrival_cortical_32k[:] = np.nan
    arrival_cortical_32k[(mask_medial_wall == 1).flatten()] = arrival_cortical.T

    save_gifti(arrival_cortical_32k[:globals.num_vertices_gifti], path_results + 'arrival_cortical_32k_L')
    save_gifti(arrival_cortical_32k[globals.num_vertices_gifti:], path_results + 'arrival_cortical_32k_R')

    Left = transforms.fslr_to_fslr(path_results + 'arrival_cortical_32k_L.func.gii',
                                   target_density ='4k',
                                   hemi = {'L'},
                                   method ='linear')
    all_data_dev[:4002, s] = Left[0].darrays[0].data
    
    Right = transforms.fslr_to_fslr(path_results + 'arrival_cortical_32k_R.func.gii',
                                   target_density ='4k',
                                   hemi = {'R'},
                                   method ='linear')
    all_data_dev[4002:, s] = Right[0].darrays[0].data

np.save(path_results + '4k_arrival_all_subjects_dev.npy', all_data_dev)
...
np.save(path_results + '4k_arrival_all_subjects_aging.npy', all_data_age)
...
for s, subid in enumerate(subject_id_dev):
    ...
    arrival_file + ' COLUMN -volume-all '+  path_results + 'temp.nii.gz'
    os.system(command)
...
np.save(path_results + '4k_subcortex_arrival_all_subjects_dev.npy', all_data_subcortex_dev)
...
np.save(path_results + '4k_subcortex_arrival_all_subjects_aging.npy', all_data_subcortex_age)
